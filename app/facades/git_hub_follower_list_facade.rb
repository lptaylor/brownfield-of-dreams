class GitHubFollowerListFacade
  def self.user_github_followers_list(current_user, github_following_or_follower)
    data = GithubService.new(current_user)
    github_followers_data = data.user_github_followers(github_following_or_follower)
